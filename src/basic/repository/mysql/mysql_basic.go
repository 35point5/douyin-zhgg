...
func NewMysqlBasicRepository(conn *gorm.DB, debug bool) domain.BasicRepository {
	...
	if debug {
		...
		conn.Create(&domain.VideoModel{Id: 1, Uid: 1, PlayUrl: domainName + staticURL + "/bear.mp4",
			CoverUrl: domainName + staticURL + "/cover.jpg", FavoriteCount: 3, CommentCount: 2,
			Title: "video1", UpdatedTime: time.Now().Add(-2 * time.Hour),
		})
		conn.Create(&domain.VideoModel{Id: 2, Uid: 2, PlayUrl: domainName + staticURL + "/song1.mp4",
			CoverUrl: domainName + staticURL + "/song1.jpg", FavoriteCount: 1, CommentCount: 1,
			Title: "video2", UpdatedTime: time.Now().Add(-time.Hour),
		})
		conn.Create(&domain.VideoModel{Id: 3, Uid: 2, PlayUrl: domainName + staticURL + "/song2.mp4",
			CoverUrl: domainName + staticURL + "/song2.jpg", FavoriteCount: 0, CommentCount: 0,
			Title: "video3", UpdatedTime: time.Now(),
		})
	}
